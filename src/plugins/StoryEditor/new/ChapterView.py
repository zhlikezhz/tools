...
class ChapterItem():
	...
	def data(self, column):
		if(column == 0):
			return units._fromUtf8(self.unitData.type)
		elif(column == 1):
	...
	def setData(self, column, value):
		if(column == 0):
			self.unitData.type = units._toUtf8(value.toString())
		elif(column == 1):
		...
		if(self.multiChoose == False):
			if(self.index != None and self.index.isValid()):
				...
				if(self.index.internalPointer() == currIndex.internalPointer()):
					...
					self.model.setData(index, QtCore.QVariant(unitData.type))

					index = self.model.index(currIndex.row(), 1, parentIndex)
					self.model.setData(index, QtCore.QVariant(unitData.sentence))
